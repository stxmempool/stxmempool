...
import { StacksBlockSummary } from '../api/stacks/stacks-api.interface';

class StacksBlockSummariesRepository {
  public async $getByBlockId(id: string): Promise<StacksBlockSummary | undefined> {
    try {
      const [summary]: any[] = await DB.query(`SELECT * from stacks_blocks_summaries WHERE id = ?`, [id]);
      if (summary.length > 0) {
  ...
  public async $saveSummary(params: { height: number, mined?: StacksBlockSummary}) {
    const blockId = params.mined?.id;
    try {
      const transactions = JSON.stringify(params.mined?.transactions || []);
      await DB.query(`
        INSERT INTO stacks_blocks_summaries (height, id, transactions, template)
        VALUE (?, ?, ?, ?)
  ...
  public async $saveTemplate(params: { height: number, template: StacksBlockSummary}) {
  ...
  public async $getIndexedSummariesId(): Promise<string[]> {
    try {
      const [rows]: any[] = await DB.query(`SELECT id from stacks_blocks_summaries`);
   ...
   */
     public async $deleteBlocksFrom(blockHeight: number) {
      ...
      try {
        await DB.query(`DELETE FROM stacks_blocks_summaries where height >= ${blockHeight}`);
      } catch (e) {
        logger.err('Cannot delete indexed blocks summaries. Reason: ' + (e instanceof Error ? e.message : e));
      }
    }
}
export default new StacksBlockSummariesRepository();