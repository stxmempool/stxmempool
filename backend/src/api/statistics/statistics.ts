...
class Statistics {
  ...
  private async runStacksStatistics(): Promise<void> {
    if (!stacksMempool.isInSync()) {
      return;
    }
    const currentMempool = stacksMempool.getMempool();
    const txPerSecond = stacksMempool.getTxPerSecond();
    ...
    memPoolArray.sort((a, b) => a.effectiveFeePerVsize - b.effectiveFeePerVsize);
    // const totalWeight = memPoolArray.map((tx) => tx.vsize).reduce((acc, curr) => acc + curr) * 4;
    const totalWeight = memPoolArray.map((tx) => tx.vsize).reduce((acc, curr) => acc + curr);

    const totalFee = memPoolArray.map((tx) => tx.feeRateAsNumber).reduce((acc, curr) => acc + curr);
